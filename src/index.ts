...
client.on(Events.InteractionCreate, async (interaction) => {
	...
	if (interaction.commandName === 'collected') {
		...
		// Header is only written once so if it doesn't appear move the log.csv file out of the folder and it should create a new one with header.
		if (!fs.existsSync('log.csv')) {
			fs.writeFileSync('log.csv', 'Time,Player,Item,Amount\n');
		}

		// First part of discord confirmation message
		...
		const materialList = ['diamond', 'gold', 'iron', 'redstone', 'lapis', 'coal'];
